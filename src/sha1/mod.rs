...
impl Sha1State {
    // Create a new Sha1State with user-provided initial values
    pub fn new(initial_state: [u32; 5]) -> Sha1State {
        Sha1State {
            state: initial_state,
        }
    }
...
impl Sha1 {
    ...
    pub fn with_state(initial_state: [u32; 5],len_bytes: u64) -> Sha1 {
        Sha1 {
            state: Sha1State::new(initial_state),
            len: len_bytes,
            blocks: Blocks {
                len: 0,
                block: [0; 64],
